...
double getLateralError(const MyReference &ref, const state_type &x, const int& IDwp,const geometry_msgs::Point& Ppreview){
    ...
    // Extract the reference of the given ID's
    vector<point2D> pref;
    // assert((0<=IDwp)&&(IDwp<=ref.x.size()-1));
    // cout<<"IDwp="<<IDwp<<"IDmin="<<IDmin<<" IDmax="<<IDmax<<endl;

    for(int i = IDmin; i<=IDmax; i++){
        point2D temp {ref.x[i],ref.y[i]};
        pref.push_back(temp);
    }
    ...
    vector<point2D> tref = transformToVehicle(pref,Xpreview);
    // Find the coordinate of local x-axis intersection to get the lateral error 
    double ym = interpolate(tref);
    return ym;
}
...
int findClosestPoint(const MyReference& ref, const geometry_msgs::Point& point, int ID){
    ...
    // if((idmin<0)||(idmin>(ref.x.size()-1))){
    //     ROS_ERROR_STREAM("IDwp="<<idmin);
    //     bool stophere = 1;
    //     sleep(10);
    // }
    // assert((0<=IDwp)&&(IDwp<=ref.x.size()-1));
